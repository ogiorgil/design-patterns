...
public class CategoryControllerTest {
    ...
    public void testPostSubcategory() throws Exception {
        ...
        when(categoryService.createSubcategory(1, subcategory)).thenReturn(category);

        mockMvc.perform(post("/category/1/sub")
    ...
    @Test
    public void testPostSubcategoryNonExistentParent() throws Exception {
        when(categoryService.createSubcategory(1, category)).thenReturn(null);
    ...
    public void testGetArticles() throws Exception {
        Article article = new Article(new Editor(), "title_here", "content_here");
        Iterable<Article> articleList = List.of(article);
        when(categoryService.getCategoryArticles(3)).thenReturn(articleList);

        mockMvc.perform(get("/category/3/articles").contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$[0].title").value("title_here"))
                .andExpect(jsonPath("$[0].content").value("content_here"));
    }
}
